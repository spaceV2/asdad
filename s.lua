...
test("loadstring", {}, function()
    ...
    elseif getgenv then
        ...
        local func = loadstring("getfenv(0).UNCtest = 1")
        if getfenv(0).UNCtest then
            assert(false, "loadstring should return a function to be called")
        end
        assert(type(func) == "function", "loadstring did not return a function")
        local a, b = pcall(func)
        assert(a, "loadstring failed to load Lua code")
        assert(getfenv(0).UNCtest, "loadstring failed to load Lua code and did not return a error")
        getfenv(0).UNCtest = nil
    end
end)
...
test("debug.getregistry", { "getregistry", "getreg", "debug.getreg" }, function()
    assert(typeof(getregistry()) == "table", "debug.getregistry did not return a table")
    assert(#getregistry() ~= 0, "debug.getregistry returned a empty table")
end)
...
test("debug.getproto", { "getproto" }, function()
    ...
    assert(not pcall(function() debug.getproto(coroutine.wrap(function() end), 1) end),
        "debug.getproto must not C functions to have protos grabbed (they don't have any)")
...
test("debug.getprotos", { "getprotos" }, function()
    ...
    assert(not pcall(function() debug.getprotos(coroutine.wrap(function() end)) end),
        "debug.getprotos must not C functions to have protos grabbed (they don't have any)")
